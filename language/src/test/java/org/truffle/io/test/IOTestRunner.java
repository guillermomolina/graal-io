...
import org.truffle.io.test.IOTestRunner.TestCase;

import com.oracle.truffle.api.dsl.NodeFactory;

public class IOTestRunner extends ParentRunner<TestCase> {
    ...
    public IOTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        ...
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", IOTestSuite.class.getSimpleName(), c.getName(), IOTestRunner.class.getSimpleName()));
        }
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        IOTestRunner suite = new IOTestRunner(testClass);
        if (args.length > 0) {
