...
public class TestRunner extends ParentRunner<TestCase> {
    ...
    public TestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        TestRunner suite = new TestRunner(testClass);
        if (args.length > 0) {
