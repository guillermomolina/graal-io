...
/**
 * IO node that performs the "+" operation, which performs addition on arbitrary precision numbers,
 ...
 * are automatically generated by the Truffle DSL. In addition, a {@link IOAddNodeGen factory class}
 * is generated that provides, e.g., {@link IOAddNodeGen#create node creation}.
 */
@NodeInfo(shortName = "+")
public abstract class IOAddNode extends IOBinaryNode {
     ...
     * the {@link #add(IOBigNumber, IOBigNumber) slow path}. Therefore, we use an
    ...
    /**
     * This is the slow path of the arbitrary-precision arithmetic. The {@link IOBigNumber} type of
     * Java is doing everything we need.
     * <p>
     * This specialization is automatically selected by the Truffle DSL if both the left and right
     * operand are {@link IOBigNumber} values. Because the type system defines an
     * {@link ImplicitCast implicit conversion} from {@code long} to {@link IOBigNumber} in
     * {@link IOTypes#castBigNumber(long)}, this specialization is also taken if the left or the
    ...
    @Specialization
    @TruffleBoundary
    protected IOBigNumber add(IOBigNumber left, IOBigNumber right) {
        return new IOBigNumber(left.getValue().add(right.getValue()));
    }

    /**
     * Specialization for TruffleString concatenation. The IO specification says that TruffleString
