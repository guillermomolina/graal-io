...
/**
 * The IO implementation of {@link NodeLibrary} provides fast access to local variables. It's used
...
public abstract class IOScopedNode extends Node {

    /**
     * Index to the the {@link IOBlockNode#getDeclaredLocalVariables() block's variables} that
     ...
     */
    @ExportMessage
    boolean accepts(@Shared("node") @Cached(value = "this", adopt = false) IOScopedNode cachedNode) {
     ...
     */
    @ExportMessage
    final Object getScope(Frame frame, boolean nodeEnter, @Shared("node") @Cached(value = "this", adopt = false) IOScopedNode cachedNode,
                    @Cached(value = "this.findBlock()", adopt = false, allowUncached = true) Node blockNode) {
        if (blockNode instanceof IOBlockNode) {
            return new VariablesObject(frame, cachedNode, nodeEnter, (IOBlockNode) blockNode);
        } else {
            return new ArgumentsObject(frame, (IOFunctionRootNode) blockNode);
        }
    }
    ...
    /**
     * Find block of this node. Traverse the parent chain and find the first {@link IOBlockNode}. If
    ...
    public final Node findBlock() {
        ...
        while (parent != null) {
            if (parent instanceof IOBlockNode) {
                break;
            }
    ...
    /**
     * Set the index to the the {@link IOBlockNode#getDeclaredLocalVariables() block's variables}
    ...
    /**
     * Scope of function arguments. This scope is provided by nodes just under a {@link IOFunctionRootNode},
     * outside of a {@link IOBlockNode block}.
     * <p>
     * The arguments declared by {@link IOFunctionRootNode#getDeclaredArguments() root node} are provided as
     * scope members.
     */
    ...
    static final class ArgumentsObject implements TruffleObject {
        ...
        protected final IOFunctionRootNode root;

        /**
         * The arguments depend on the current frame and root node.
         */
        ArgumentsObject(Frame frame, IOFunctionRootNode root) {
            this.frame = frame;
            this.root = root;
        }
         ...
         */
        @ExportMessage
        boolean accepts(@Cached(value = "this.root", adopt = false) IOFunctionRootNode cachedRoot) {
        ...
        Object getMembers(boolean includeInternal) {
            IOWriteLocalVariableNode[] writeNodes = root.getDeclaredArguments();
        ...
        int findArgumentIndex(String member) {
            TruffleString memberTS = IOSymbols.fromJavaString(member);
            IOWriteLocalVariableNode[] writeNodes = root.getDeclaredArguments();
            for (int i = 0; i < writeNodes.length; i++) {
                IOWriteLocalVariableNode writeNode = writeNodes[i];
    ...
    static final class VariablesObject implements TruffleObject {
        ...
        protected final IOScopedNode node;  // the current node
        final boolean nodeEnter;            // whether the node was entered or is about to be exited
        protected final IOBlockNode block;  // the inner-most block of the current node

        VariablesObject(Frame frame, IOScopedNode node, boolean nodeEnter, IOBlockNode blockNode) {
            this.frame = frame;
            this.node = node;
         ...
         */
        @ExportMessage
        boolean accepts(@Cached(value = "this.node", adopt = false) IOScopedNode cachedNode,
         ...
         */
        @ExportMessage
        Object toDisplayString(boolean allowSideEffects, @Shared("block") @Cached(value = "this.block", adopt = false) IOBlockNode cachedBlock,
            ...
            if (parentBlock instanceof IOBlockNode) {
                ...
                return ((IOFunctionRootNode) parentBlock).getTSName();
            }
        }
        ...
        boolean hasScopeParent(
                        @Shared("block") @Cached(value = "this.block", adopt = false) IOBlockNode cachedBlock,
                        @Shared("parentBlock") @Cached(value = "this.block.findBlock()", adopt = false, allowUncached = true) Node parentBlock) {
            // Cache the parent block for the fast-path access
            return parentBlock instanceof IOBlockNode;
        }
        ...
        @ExportMessage
            ...
            return new VariablesObject(frame, cachedBlock, true, (IOBlockNode) parentBlock);
        }
        ...
        SourceSection getSourceLocation() {
            ...
            if (parentBlock instanceof RootNode) {
                ...
                assert parentBlock instanceof IOFunctionRootNode : String.format("In IOLanguage we expect IOFunctionRootNode, not %s", parentBlock.getClass());
                return ((IOFunctionRootNode) parentBlock).getSourceSection();
            } else {
        ...
        static final class WriteMember {
            ...
            @Specialization(limit = "LIMIT", guards = {"cachedMember.equals(member)"})
                        static void doCached(VariablesObject receiver, String member, Object value,
                            ...
                            @Cached(value = "receiver.findWriteNode(member)", adopt = false) IOWriteLocalVariableNode writeNode) throws UnknownIdentifierException, UnsupportedMessageException {
            ...
            static void doGeneric(VariablesObject receiver, String member, Object value) throws UnknownIdentifierException, UnsupportedMessageException {
                IOWriteLocalVariableNode writeNode = receiver.findWriteNode(member);
                doWrite(receiver, member, writeNode, value);
            }

            private static void doWrite(VariablesObject receiver, String member, IOWriteLocalVariableNode writeNode, Object value) throws UnknownIdentifierException, UnsupportedMessageException {
                if (writeNode == null) {
                    throw UnknownIdentifierException.create(member);
                }
                if (receiver.frame == null) {
        ...
        @ExportMessage
        Object getMembers(boolean includeInternal,
                        @Cached(value = "this.block.getDeclaredLocalVariables()", adopt = false, dimensions = 1, allowUncached = true) IOWriteLocalVariableNode[] writeNodes,
        ...
        int findSlot(String member) {
            IOWriteLocalVariableNode writeNode = findWriteNode(member);
            if (writeNode != null) {
         ...
         */
        IOWriteLocalVariableNode findWriteNode(String member) {
            TruffleString memberTS = IOSymbols.fromJavaString(member);
            IOWriteLocalVariableNode[] writeNodes = block.getDeclaredLocalVariables();
    ...
    static final class KeysArray implements TruffleObject {
        ...
        @CompilationFinal(dimensions = 1) private final IOWriteLocalVariableNode[] writeNodes;
         ...
         */
        KeysArray(IOWriteLocalVariableNode[] writeNodes, int variableIndex, int parentBlockIndex) {
    ...
    /**
     * Representation of a variable based on a {@link IOWriteLocalVariableNode write node} that
    ...
    static final class Key implements TruffleObject {
        ...
        private final IOWriteLocalVariableNode writeNode;

        Key(IOWriteLocalVariableNode writeNode) {
        ...
        SourceSection getSourceLocation() throws UnsupportedMessageException {
            ...
            IOExpressionNode nameNode = writeNode.getNameNode();
