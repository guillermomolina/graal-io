...
/**
 * The builtin type definitions for IO. IO has no custom types, so it is not possible
 * for a guest program to create new instances of IOType.
 * <p>
 ...
 * <p>
 * In order to assign types to guest language values, IO values implement
 ...
 * assigned using language views. See {@link IOLanguage#getLanguageView}.
 */
@ExportLibrary(InteropLibrary.class)
public final class IOType implements TruffleObject {
     ...
     * additional checks need to be performed (similar to number checking for IOBigNumber).
     */
    public static final IOType NUMBER = new IOType("Number", (l, v) -> l.fitsInLong(v) || v instanceof IOBigNumber);
    public static final IOType NIL = new IOType("NIL", (l, v) -> l.isNull(v));
    public static final IOType STRING = new IOType("String", (l, v) -> l.isString(v));
    public static final IOType BOOLEAN = new IOType("Boolean", (l, v) -> l.isBoolean(v));
    public static final IOType METHOD = new IOType("Method", (l, v) -> l.isExecutable(v));
    public static final IOType LIST = new IOType("List", (l, v) -> l.hasArrayElements(v));
    public static final IOType OBJECT = new IOType("Object", (l, v) -> l.hasMembers(v));
     ...
     * example, an object might be a function. In IO we decided to make functions,
     * functions and not objects.
     */
    @CompilationFinal(dimensions = 1) public static final IOType[] PRECEDENCE = new IOType[]{NIL, NUMBER, STRING, BOOLEAN, METHOD, LIST, OBJECT};

    private final String name;
    private final TypeCheck isInstance;

    /*
     * We don't allow dynamic instances of IOType. Real languages might want to expose this for
     * types that are user defined.
     */
    private IOType(String name, TypeCheck isInstance) {
        this.name = name;
     ...
     * cast {@link IOType} to a constant.
     */
    ...
    /*
     * All IOTypes are declared as interop meta-objects. Other example for meta-objects are Java
    ...
    /*
     * IO does not have the notion of a qualified or simple name, so we return the same type name
     * for both.
     */
     ...
     * IOIsInstanceBuiltin isInstance} builtin. It checks whether a given value, which might be a
     * primitive, foreign or IO value is of a given IO type. This allows other languages to make
    ...
    static class IsMetaInstance {
        ...
        static boolean doCached(IOType type, Object value,
                        @Cached("type") IOType cachedType,
        ...
        static boolean doGeneric(IOType type, Object value) {
