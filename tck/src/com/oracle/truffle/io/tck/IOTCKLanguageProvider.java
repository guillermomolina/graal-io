...
public class IOTCKLanguageProvider implements LanguageProvider {
    private static final String ID = "io";
    ...
    private static final String[] PATTERN_EXPRESSIONS = {
                    "function %s() {r = 0;\n%s\nreturn r;\n}",
                    "function %s(p1) {r = 0;\n%s\nreturn r;\n}",
    };
    ...
    public Collection<? extends Snippet> createExpressions(Context context) {
        final Collection<Snippet> res = new ArrayList<>();
        res.add(createExpression(context, "if", "iffnc", "if ({1}) '{'\n{0}=1;\n'}' else '{'\n{0}=0;\n'}'", TypeDescriptor.NUMBER, TypeDescriptor.BOOLEAN));
        res.add(createExpression(context, "while", "whilefnc", "while ({1}) '{'break;\n'}'", TypeDescriptor.NUMBER, TypeDescriptor.BOOLEAN));
        res.add(createExpression(context, "assign", "assignfnc", "{1} = {0};", TypeDescriptor.ANY, TypeDescriptor.ANY));
    ...
    public Collection<? extends Snippet> createScripts(Context context) {
        final Collection<Snippet> res = new ArrayList<>();
        res.add(loadScript(context, "resources/Ackermann.io", TypeDescriptor.NULL, null));
        res.add(loadScript(context, "resources/Fibonacci.io", TypeDescriptor.NULL, null));
        return Collections.unmodifiableCollection(res);
    }
    ...
    public Collection<? extends Source> createInvalidSyntaxScripts(Context context) {
        try {
            ...
            res.add(createSource("resources/InvalidSyntax01.io"));
            res.add(createSource("resources/InvalidSyntax02.io"));
            return Collections.unmodifiableCollection(res);
    ...
    private static Snippet createExpression(
                    final Context context,
                    final String id,
                    final String functionName,
        ...
        final Value fnc = eval(context,
                        String.format(PATTERN_EXPRESSIONS[paramTypes.length], functionName, formattedExpression),
                        functionName);
    ...
    private static Source createSource(final String resourceName) throws IOException {
        ...
        try (Reader in = new InputStreamReader(IOTCKLanguageProvider.class.getResourceAsStream(resourceName), "UTF-8")) {
